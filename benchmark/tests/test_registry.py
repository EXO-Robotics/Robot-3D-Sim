...
from exo_bench.hashing import require_sha256
...
def test_aliases_resolve_to_hash_bound_pair() -> None:
    ...
    assert artifacts.robot_id == "exo.h1-locomotion.v1"
    ...
    assert artifacts.reset_profile_id == "exo.reset-perturbation.basic.v1"
...
def test_pinned_artifact_hashes_match() -> None:
    ...
    assert len(hashes["reset_profile_sha256"]) == 64


def test_pending_artifact_hash_is_never_accepted() -> None:
    with pytest.raises(ValueError, match="concrete lowercase SHA-256"):
        require_sha256("pending-runtime-verification", "model_tree_sha256")
